...
class StateHousingPolicy(PolicyBase):
    ...
    def __init__(self):
        ...
        self.effects.append(SimulationConnection('poor_mood', '0.05 + (0.05 * x)'))
        self.effects.append(SimulationConnection('equality', '0.0 + (0.2 * x)'))
        self.effects.append(SimulationConnection('socialist_mood', '0.0 + (0.20 * x)'))
        self.effects.append(SimulationConnection('poverty_rate', '0.0 - (0.17 * x)'))
        self.effects.append(SimulationConnection('_low_income', '0.0 + (0.17 * x)'))
        # _global_socialism, 0.015 + (0.02 * x), 12
        self.effects.append(SimulationConnection('private_housing', '0.0 - (0.8 * x)', 4))
