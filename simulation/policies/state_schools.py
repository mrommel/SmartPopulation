...
class StateSchoolsPolicy(PolicyBase):
	...
	def __init__(self):
		...
		# connections:
		self.effects.append(Effect('poor_mood', '0.04 + (0.11 * x)'))
		self.effects.append(Effect('socialist_mood', '0.0 + (0.20 * x)'))
		self.effects.append(Effect('education', '0.3 * (x ** 0.6) + 0.07', 8))
		self.effects.append(Effect('poverty_rate', '-0.08 - (0.10 * x)'))
		self.effects.append(Effect('state_employees_mood', '0.0 + (0.14 * x)'))
		...
		self.effects.append(Effect('parents_freq', '0.0 + (0.025 * x)', 8))
		self.effects.append(Effect('private_schools', '0.0 - (0.8 * x)', 4))
