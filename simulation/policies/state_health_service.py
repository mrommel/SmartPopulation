...
class StateHealthServicePolicy(PolicyBase):
	...
	def __init__(self):
		...
		# _default_,0.6;TobaccoUse,0+(0.16*x);Environment,0.2-(0.1*x);Alcoholism,0.1+(0.1*x);Wages,-0.1+(0.2*x)
		...
		# connections:
		self.effects.append(Effect('poor_mood', '0.05 + (0.15 * x)'))
		self.effects.append(Effect('capitalist_mood', '-0.02 - (0.10 * x)'))
		self.effects.append(Effect('wealthy_mood', '0.00 - (0.10 * x)'))
		self.effects.append(Effect('socialist_mood', '0.05 + (0.11 * x)'))
		self.effects.append(Effect('health', '0.25 * (x ** 0.6) + 0.05', 4))
		self.effects.append(Effect('retired_mood', '0.0 + (0.16 * x)'))
		self.effects.append(Effect('unemployment', '0.0 - (0.19 * x)'))
		self.effects.append(Effect('state_employees_mood', '0.0 + (0.22 * x)'))
		self.effects.append(Effect('state_employees_freq', '0.0 + (0.1 * x)'))
		# state_employees_income,0+(0.09*x)
		# _global_socialism,0+(0.052*x),4
		self.effects.append(Effect('private_healthcare', '0.0 - (0.85*x)', 4))
